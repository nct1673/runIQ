...
/** Line chart of distance per calendar week, on a light grid -- a gridded
 * line reads as a trend at a glance, which a bar-per-week view didn't.
 * Single series (distance), so identity is carried by the title, not a
 * legend -- color is the fixed "mileage" amber used for both mileage
 * views (weekly/monthly). */
export default function WeeklyMileageChart({ data }: { data: WeeklyMileagePoint[] }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="mt-1 flex text-[10px] text-text-muted" style={{ pointerEvents: "none" }}>
        {data.map((d, i) => (
          <div key={d.week_start} style={{ width: `${barSlot}%` }} className="truncate text-center">
            {i % Math.ceil(data.length / 6) === 0 ? weekLabel(d.week_start) : ""}
          </div>
        ))}
      </div>
      ...
      {hovered != null && (
        ...
        >
          ...
          <div className="text-text-muted">
            week of {weekLabel(data[hovered].week_start)} · {data[hovered].runs} run
